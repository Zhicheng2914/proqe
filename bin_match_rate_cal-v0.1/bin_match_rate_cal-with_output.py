...
def compute_matching_probability_and_count(bin_info):
    ...
    for _, bucket_row in target_bucket.iterrows():
        for _, kickstand_row in kickstands.iterrows():
            if can_match(bucket_row, kickstand_row):
                match_count += 1
                matched_sn_pairs.append((bucket_row['SN'], kickstand_row['SN']))  # 将匹配的SN对添加到列表中
                break  # 找到匹配后，跳出内部循环

    total_buckets = len(target_bucket)
    ...
    # 输出匹配的SN到CSV文件
    matched_sn_df = pd.DataFrame(matched_sn_pairs, columns=['Bucket_SN', 'Kickstand_SN'])
    matched_sn_df.to_csv('matched_sn.csv', index=False)
